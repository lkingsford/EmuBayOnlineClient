...
export class Board extends State.State {
    ...
    public resizeToWidth(newWidth: number): void {
        ...
        this.playfield!.scale = new PIXI.Point(
            this.view_scale,
            this.view_scale
        );
        this.app.renderer.resize(
            2000 * this.view_scale,
            2000 * this.view_scale
        );
    }
    ...
    public drawMap(gamestate: IEmuBayState, ctx: Ctx): void {
        ...
        gamestate.companies.forEach((co, i) => {
            if (co.home) {
                ...
                if (i == 2) {
                    special_offset = Board.TILE_WIDTH * -0.3;
                }
                if (i == 1) {
                    special_offset = Board.TILE_WIDTH * 0.3;
                }
                homeSprite.x =
                    Board.TILE_WIDTH * co.home.x +
                    Board.OFFSET_X +
                    special_offset;
                homeSprite.y =
                    Board.TILE_HEIGHT * co.home.y +
                    Board.OFFSET_Y +
        ...
        gamestate.track.forEach((xy) => {
            ...
            location.push(
                Board.cubeTextures[xy.narrow ? CubeTexture.Narrow : xy.owner!]
            );
            CubeLocations[`${xy.x},${xy.y}`] = location;
        });
        ...
        MAP.forEach((terrain) => {
            terrain.locations.forEach((xy) => {
                ...
                cubesToPlace.forEach((t, i) => {
                    ...
                    let x =
                        Board.TILE_WIDTH * xy.x +
                        Board.OFFSET_X +
                        Math.sin(angle) * this.cube_place_radius;
                    let y =
                        Board.TILE_HEIGHT * xy.y +
                        Board.OFFSET_Y +
                        (xy.x % 2 == 0 ? Board.TILE_HEIGHT / 2 : 0) +
        ...
        if (stage == PseudoStage.takeResources) {
            ...
            let allowedSpaces = getTakeResourceSpaces(
                gamestate,
                gamestate.toAct!
            );
            allowedSpaces.forEach((xy) => {
    ...
    static getTextures(resources: { [index: string]: PIXI.LoaderResource }) {
        ...
        for (let ix = 0; ix < 3; ix++) {
            for (let iy = 0; iy < 2; iy++) {
                ...
                Board.tileTextures[ix + iy * 3] = new PIXI.Texture(
                    resources["map_tiles"].texture
                        .baseTexture as PIXI.BaseTexture,
                    new PIXI.Rectangle(
                        srcX,
                        srcY,
                        Board.TEXTURE_TILE_WIDTH,
                        Board.TEXTURE_TILE_HEIGHT
                    )
                );
            }
        }

        {
            ...
            Board.canChooseTexture = new PIXI.Texture(
                resources["map_tiles"].texture.baseTexture as PIXI.BaseTexture,
                new PIXI.Rectangle(
                    srcX,
                    srcY,
                    Board.TEXTURE_TILE_WIDTH,
        ...
        for (let ix = 0; ix < 7; ix++) {
            ...
            // TODO: Maybe this shouldn't be hardcoded yaknow
            Board.companyTextures[ix] = new PIXI.Texture(
                resources["map_tiles"].texture.baseTexture as PIXI.BaseTexture,
                new PIXI.Rectangle(
                    srcX,
                    srcY,
                    Board.COMPANY_TILE_WIDTH,
                    Board.COMPANY_TILE_HEIGHT
                )
            );
        }
        ...
        for (let ix = 0; ix < 5; ix++) {
            ...
            Board.cubeTextures[ix] = new PIXI.Texture(
                resources["map_tiles"].texture.baseTexture as PIXI.BaseTexture,
                new PIXI.Rectangle(
                    srcX,
                    srcY,
                    Board.CUBE_TILE_WIDTH,
                    Board.CUBE_TILE_HEIGHT
                )
            );
        }
    }
